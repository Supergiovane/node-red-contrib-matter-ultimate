...
2. 为每个设备或虚拟功能添加一个 **Expose to matter** 节点。
...
每个 Expose to matter 节点将**一个 KNX 设备公开为 Matter 设备**：已配对的控制器（Alexa、Google Home、Apple Home...）会以您输入的名称看到它，随时可通过 App 和语音控制。将它指向一个 **Matter 桥接器**配置节点（真正的桥接器，只需配对一次 - 配对二维码在那里），然后在流程中随意添加任意数量的设备节点。

这与 _Matter Device_ 节点方向相反：那里是 KNX 控制 Matter 设备，这里是 Matter 控制器控制 KNX。
