...
module.exports = (RED) => {
  function matterBridgeConfig (config) {
    ...
    node.sysLogger = null
    try {
      node.sysLogger = new LoggerClass({ loglevel: node.loglevel, setPrefix: 'matter-ultimate-bridge-config.js' })
    ...
    const bindEngineEvents = (engine) => {
      ...
      engine.on('command', (command) => {
        try {
          ...
          node.sysLogger?.warn(`matter-ultimate-bridge-config: command routing error: ${error.message}`)
        }
      })
      ...
      reconcileTimer = setTimeout(() => {
        (async () => {
          try {
            ...
            node.sysLogger?.warn(`matter-ultimate-bridge-config: reconcile error: ${error.message}`)
          }
        })().catch((error) => node.sysLogger?.warn(`matter-ultimate-bridge-config: reconcile callback error: ${error.message}`))
      }, 1500)
    }
    ...
    const ensureEngineStarted = async () => {
      try {
        ...
        const existing = runningBridges.get(node.id) || require('./utils/legacyMatterHandoff').takeBridge(RED, node.id)
        if (existing) runningBridges.set(node.id, existing)
        ...
        node.sysLogger?.error(`matter-ultimate-bridge-config: ensureEngineStarted error: ${error.message}`)
      }
    }
    ...
    startupTimer = setTimeout(() => {
      ...
      })().catch((error) => node.sysLogger?.error(`matter-ultimate-bridge-config: startup callback error: ${error.message}`))
    }, 5000)
    ...
    node.registerDevice = (client) => {
      try {
        ...
        node.sysLogger?.warn(`matter-ultimate-bridge-config: registerDevice error: ${error.message}`)
      }
    }
  ...
  RED.nodes.registerType('matter-ultimate-bridge-config', matterBridgeConfig)
}
