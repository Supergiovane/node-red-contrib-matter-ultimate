...
// One KNX device exposed as a Matter device. It points to a matter-ultimate-bridge-config node
...
module.exports = function (RED) {
  ...
  function matterUltimateBridge (config) {
    ...
    const reportMatterError = (context, error) => {
      ...
      try { RED.log.error(`matterUltimateBridge: ${context}: ${message}`) } catch (logError) { /* empty */ }
    }
    ...
    node.handleMatterCommand = (command) => {
      try {
        ...
        if (node.enableNodePINS) {
          try {
            node.send({
              topic: topicMapping.isTopicMode(config) ? (commandRoutes.get(command.fn)?.ga || node.name) : node.name,
    ...
    node.handleSend = (msg) => {
      try {
        ...
        routes.forEach((route) => {
          try {
            const value = topicMapping.readValue(msg, dptlib, route.dpt)
    ...
    if (node.serverKNX) {
      try {
        ...
        RED.log.error(`matterUltimateBridge: register KNX client error ${error.message}`)
      }
    }
    if (node.serverMatterBridge) {
      try {
        ...
        RED.log.error(`matterUltimateBridge: register bridge device error ${error.message}`)
      }
    } else {
      node.setNodeStatus({ fill: 'red', shape: 'ring', text: 'No Matter bridge selected' })
    }
    ...
    node.on('input', async (msg, send, done) => {
      if (topicMapping.routeInput(node, config, msg, done, Array.from(statusRoutes.keys()))) return
      if (!node.enableNodePINS) {
        if (done) done()
        return
      }
  ...
  RED.nodes.registerType('matterUltimateBridge', matterUltimateBridge)
}
