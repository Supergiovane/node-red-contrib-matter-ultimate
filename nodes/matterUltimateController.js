const topicMapping = require('./utils/topicMapping')
...
module.exports = function (RED) {
  if (!matterCommandGateEndpointRegistered) {
    RED.httpAdmin.post('/matterUltimate/KNXUltimateMatterControllerEnable', RED.auth.needsPermission('matter-ultimate-config.write'), (req, res) => {
      try {
  ...
  function matterUltimateController(config) {
    ...
    // matter-ultimate-config creates its matterManager asynchronously after the deploy.
      ...
      hueManager: {
        writeHueQueueAdd: (_lightID, _state, _operation) => {
          try {
            ...
            RED.log.error('matterUltimateController: writeHueQueueAdd shim: ' + (error && error.message))
          }
        },
        deleteHueQueue: () => { }
      },
    ...
    if (config.colorAtSwitchOnDayTime.indexOf('#') !== -1) {
      ...
      try {
        ...
        RED.log.error(`matterUltimateController:  config.colorAtSwitchOnDayTime = JSON.parse(config.colorAtSwitchOnDayTime): ${error.message} : ${error.stack || ''} `)
    ...
    if (config.colorAtSwitchOnNightTime.indexOf('#') !== -1) {
      ...
      try {
        ...
        RED.log.error(`matterUltimateController:  config.colorAtSwitchOnDayTime = JSON.parse(config.colorAtSwitchOnNightTime): ${error.message} : ${error.stack || ''} `)
    ...
    node.writeHueState = function writeHueState(_state) {
      ...
      (async () => {
        try {
          ...
          RED.log.debug(`matterUltimateController: preset grouped_light children before group on. Group=${node.hueDevice} lights=${Array.isArray(groupLights) ? groupLights.length : 0}`)
          ...
          RED.log.debug(`matterUltimateController: turning on grouped_light after children preset. Group=${node.hueDevice}`)
          node.serverHue.hueManager.writeHueQueueAdd(node.hueDevice, groupedLightState, defaultOperation)
        } catch (error) {
          RED.log.debug(`matterUltimateController: node.writeHueState fallback to grouped_light write: ${error.message}`)
          node.serverHue.hueManager.writeHueQueueAdd(node.hueDevice, _state, defaultOperation)
        }
      })()
    }
    ...
    node.deleteHueStateQueue = function deleteHueStateQueue() {
      ...
      (async () => {
        try {
          ...
          RED.log.debug(`matterUltimateController: node.deleteHueStateQueue: ${error.message}`)
        }
      })()
    }
    ...
    node.handleSend = (msg) => {
      ...
      if (msg.knx.event !== 'GroupValue_Read' && node.currentHUEDevice !== undefined) {
        ...
        try {
                  ...
                  if (msg.payload === true) {
                    if (node.timerCheckForFastLightSwitch === null) {
                      node.timerCheckForFastLightSwitch = setTimeout(() => {
                        node.DayTime = false
                        RED.log.debug('matterUltimateController: node.timerCheckForFastLightSwitch: set daytime to false after node.timerCheckForFastLightSwitch elapsed')
                        node.timerCheckForFastLightSwitch = null
                      }, 10000) // 10 seconds
                    } else {
                      if (config.restoreDayMode === 'setDayByFastSwitchLightALL') {
                        ...
                        if (node.serverKNX && config.GADaylightSensor !== undefined && config.GADaylightSensor !== '') {
                          ...
                          RED.log.debug(`matterUltimateController: node.timerCheckForFastLightSwitch: set daytime the group address ${config.GADaylightSensor}`)
                      ...
                      node.DayTime = true
                      RED.log.debug('matterUltimateController: node.timerCheckForFastLightSwitch: set daytime to true')
                    }
                  }
                }
              }
          ...
          RED.log.error(`matterUltimateController: node.handleSend:  if (msg.knx.event !== "GroupValue_Read"): ${error.message} : ${error.stack || ''} `)
        }
      }
      ...
      try {
        ...
        RED.log.error(`matterUltimateController: node.handleSend: if (msg.knx.event === "GroupValue_Read" && node.currentHUEDevice !== undefined): ${error.message} : ${error.stack || ''} `)
      }
    }
  ...
  */
    ...
    node.handleSendHUE = async (_event) => {
      ...
      try {
        ...
        RED.log.error(`matterUltimateController: node.handleSendHUE = (_event): ${error.stack}`)
      }
      // })();
    }
    ...
    // Called by matter-ultimate-config on every attribute report of the subscription.
    node.handleSendMatter = (_event) => {
      try {
        ...
        RED.log.error(`matterUltimateController: node.handleSendMatter: ${error.message}`)
      }
    }
    ...
    */
    ...
    if (node.serverHue) {
      try {
        ...
        RED.log.error('matterUltimateController: if (node.serverKNX): ' + error.message)
      }
    }
