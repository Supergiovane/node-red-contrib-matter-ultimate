...
const decodeKnxBoolean = (msg, dpt) => {
  if (msg && msg.knx && Buffer.isBuffer(msg.knx.rawValue)) {
    return !!topicMapping.readValue(msg, dptlib, dpt || '1.001')
  }
...
const setupDoorLockProfile = (RED, node, config) => {
  ...
  const writeKnxState = (state, outputtype = 'write') => {
    ...
    if (topicMapping.publishState(node, config, ga, state)) return
  ...
  const queueCommand = (locked, source = 'knx') => {
    ...
    if (queued && typeof queued.catch === 'function') {
      queued.catch((error) => {
        RED.log.error(`matterUltimateController DoorLock command: ${error.message}`)
  ...
  node.handleSend = (msg) => {
    try {
      ...
      RED.log.error(`matterUltimateController DoorLock KNX: ${error.message}`)
      setStatus('red', 'ring', error.message)
    }
  }
  ...
  node.handleSendMatter = (event) => {
    try {
      ...
      RED.log.error(`matterUltimateController DoorLock Matter: ${error.message}`)
    }
  }
  ...
  node.handleMatterNodeInitialized = () => {
    try {
      ...
      RED.log.error(`matterUltimateController DoorLock initial state: ${error.message}`)
    }
  }
