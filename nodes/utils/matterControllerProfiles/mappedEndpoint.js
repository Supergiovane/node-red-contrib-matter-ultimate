...
const parseMappings = (value, topicMode = false) => {
  try {
    ...
    return parsed.filter((mapping) => {
      if (!mapping || !mapping.target) return false
      if (topicMode) return typeof mapping.ga === 'string' && mapping.ga.trim() !== ''
      if (!isValidGroupAddress(mapping.ga) || !mapping.dpt) return false
...
const setupMappedEndpointProfile = (RED, node, config, options = {}) => {
  ...
  const sendKnx = (mapping, payload, outputtype = 'write') => {
    if (topicMapping.publishState(node, config, mapping.ga, payload)) return true
  ...
  node.handleSend = (msg) => {
    try {
      ...
      matches.filter((mapping) => mapping.direction === 'command').forEach((mapping) => {
        const value = topicMapping.readValue(msg, dptlib, mapping.dpt)
        enqueue(mapping, value)
      })
      status('green', 'dot', 'KNX→Matter')
    } catch (error) {
      RED.log.error(`matterUltimateController mapped KNX: ${error.message}`)
  ...
  node.handleSendMatter = (event) => {
    try {
      ...
      RED.log.error(`matterUltimateController mapped Matter: ${error.message}`)
      status('red', 'ring', error.message)
    }
  }
