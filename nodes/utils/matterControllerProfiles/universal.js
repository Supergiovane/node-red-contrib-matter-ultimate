...
const setupUniversalProfile = (RED, node, config) => {
  ...
  const sendKnx = (ga, payload, dpt, outputtype = 'write') => {
    if (topicMapping.publishState(node, config, ga, payload)) return
  ...
  const scanBatterySnapshot = (nodeId) => {
    ...
    selected.forEach((detail) => {
      try {
        ...
        RED.log.debug(`matterUltimateController Universal battery snapshot: ${error.message}`)
      }
    })
    syncKnxBatteryOutputs()
    refreshBatteryStatus()
  ...
  node.handleSendMatter = (event) => {
    try {
      ...
      RED.log.error(`matterUltimateController Universal attribute: ${error.message}`)
  ...
  node.handleMatterClusterEvent = (event) => {
    try {
      ...
      RED.log.error(`matterUltimateController Universal cluster event: ${error.message}`)
      status('red', 'ring', error.message)
    }
  }
  ...
  node.handleMatterNodeInitialized = (event) => {
    try {
      ...
      RED.log.error(`matterUltimateController Universal initialization: ${error.message}`)
      status('red', 'ring', error.message)
    }
  }
  ...
  if (node.serverMatter) {
    ...
    if (universalService === 'batteryMonitor') {
      Promise.resolve().then(() => scanBatterySnapshot()).catch((error) => {
        RED.log.error(`matterUltimateController Universal battery startup: ${error.message}`)
      })
    }
  } else {
