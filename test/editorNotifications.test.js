...
describe('Editor notifications', () => {
  ...
  it('loads the global guard before the package migration notifications', () => {
    const pluginPath = pkg['node-red'].plugins.matterUltimateMigration
    const plugin = fs.readFileSync(path.join(__dirname, '..', pluginPath), 'utf8')
    const guard = plugin.indexOf('editorNotifications.js')

    assert(guard >= 0)
    assert(guard < plugin.indexOf('ultimatePackageMigration.js'))
  })
})
