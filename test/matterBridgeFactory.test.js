...
describe('matterUltimateBridge editor layout', () => {
  const projectRoot = path.join(__dirname, '..')
  const editor = fs.readFileSync(path.join(projectRoot, 'nodes', 'matterUltimateBridge.html'), 'utf8')
  ...
  it('places the PIN selector outside the tabs and reveals contextual help only when enabled', () => {
    ...
    expect(editor).to.include("$pinSelect.on('change.matterUltimateBridge', updateFlowHelpVisibility)")
  })
  ...
  it('keeps flow-help strings and documentation aligned in every supported locale', () => {
    for (const locale of ['en', 'it', 'de', 'fr', 'es', 'zh-CN']) {
      const messages = require(path.join(projectRoot, 'nodes', 'locales', locale, 'matterUltimateBridge.json')).matterUltimateBridge
      ...
      const help = fs.readFileSync(path.join(projectRoot, 'nodes', 'locales', locale, 'matterUltimateBridge.html'), 'utf8')
...
describe('matterUltimateBridge – Room Air Conditioner and Door Lock routing', () => {
  it('routes Matter commands to their configured KNX GAs and KNX statuses back to Matter', () => {
    ...
    require('../nodes/matterUltimateBridge.js')(RED)
...
describe('matterUltimateBridge – optimistic cover feedback', () => {
