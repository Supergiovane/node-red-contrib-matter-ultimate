...
describe('Matter QR scanner editor helpers', () => {
  ...
  it('ships a browser decoder and the complete editor lifecycle', () => {
    ...
    const editor = fs.readFileSync(path.join(projectRoot, 'nodes/matter-ultimate-config.html'), 'utf8')
    ...
    expect(editor).to.include('matterUltimate/KNXUltimateMatterPairProgress?serverId=')
  ...
  it('keeps scanner messages and help aligned in every supported locale', () => {
    ...
    locales.forEach((locale) => {
      const messages = require(path.join(projectRoot, 'nodes/locales', locale, 'matter-ultimate-config.json'))
      const properties = messages['matter-ultimate-config'].properties
      requiredKeys.forEach((key) => expect(properties[key], `${locale}:${key}`).to.be.a('string').and.not.equal(''))

      const help = fs.readFileSync(path.join(projectRoot, 'nodes/locales', locale, 'matter-ultimate-config.html'), 'utf8')
  ...
  it('registers an operation-isolated local commissioning progress endpoint', () => {
    const runtime = fs.readFileSync(path.join(projectRoot, 'nodes/matterUltimate-admin.js'), 'utf8')
    expect(runtime).to.include("RED.httpAdmin.get('/matterUltimate/KNXUltimateMatterPairProgress'")
  ...
  it('preserves saved configuration through an untouched open/save/reopen round trip', () => {
    ...
    const context = {
      ...
      navigator: {},
      window: {
        MatterStandaloneKNXUltimateMatterQrScanner: require('../resources/matterQrScanner'),
